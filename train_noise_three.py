...
from utils import TNMTrain, MTrain, TCEval, TMEachEval, PGD_Eval, CEval, MEachEval
...
if __name__ == "__main__":
    ...
    TNMTrain(t_model_group, args.warm_epoch, args.train_epoch, header, args.noise_type, 0.0, args.train_var, args.rate_max, args.rate_zero, args.write_var, args.train_attack_runs, args.attack_dist, logger=logger, verbose=args.verbose, **kwargs)
    ...
    model.clear_mask()
    model.to_first_only()
    performance = MEachEval(model_group, "Gaussian", args.attack_dist, 1, 0, 0, **kwargs)
    logger.info(f"No mask noise acc: {performance:.4f}, distance: {args.attack_dist}")
