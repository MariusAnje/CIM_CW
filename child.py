...
from modules import SConv2d, SLinear, SBatchNorm2d, SMaxpool2D, SModel
...
def build_graph(input_shape, arch_paras):
    ...
    for layer_paras in arch_paras:
        ...
        cell.bn = SBatchNorm2d(num_filters)
        padding_height, out_height = compute_padding(
                    out_height,
                    pool_size,
                    pool_stride,
        ...
        cell.pool = SMaxpool2D(pool_size)
...
class CNN(SModel):
    def __init__(self, graph, num_classes, do_bn=False):
        ...
        self.fc2 = SLinear(256, num_classes)
    ...
    def forward(self, x, quan_paras=None):
        ...
        for cell in self.graph:
            ...
            if quan_paras is not None:
                weight, bias, noise = conv.op.weight, conv.op.bias, conv.noise
                weight = quantize(
                            weight,
    ...
    def quantize_weight(self, num_int_bits, num_frac_bits):
        for cell in self.graph:
            conv = getattr(self, 'conv_{}'.format(cell.id))
            conv.op.weight = quantize(conv.op.wieght, num_int_bits, num_frac_bits)
            conv.op.bias = quantize(conv.op.bias, num_int_bits, num_frac_bits)
