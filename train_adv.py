...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), f"saved_B_{header}_{args.attack_runs}_{args.attack_lr}.pt")
    model.clear_noise()
    print(f"No mask no noise: {CEval():.4f}")
    state_dict = torch.load(f"saved_B_{header}_{args.attack_runs}_{args.attack_lr}.pt")
    model.load_state_dict(state_dict)
    model.clear_mask()
    model.to_first_only()
