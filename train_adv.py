...
def ATrain(epochs, header, verbose=False):
    best_acc = 0.0
    for i in range(epochs):
        model.train()
        running_loss = 0.
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), f"saved_B_{header}_{args.attack_runs}_{args.attack_lr}_{args.attack_c}.pt")
    model.clear_noise()
    print(f"No mask no noise: {CEval():.4f}")
    state_dict = torch.load(f"saved_B_{header}_{args.attack_runs}_{args.attack_lr}_{args.attack_c}.pt")
    model.load_state_dict(state_dict)
    model.clear_mask()
    model.to_first_only()
