...
def fit(model, optimizer=None, train_data=None, val_data=None, epochs=40,
        verbosity=0, quan_paras=None, lr_schedule=None, dev_var = 0):
    acc = []
    loss = []
    timer = Timer()
    timer.reset()
    for epoch in range(epochs):
        ...
        if train_data is not None:
            ...
            train_loss, train_acc = epoch_fit(
                model, train_data, optimizer, verbosity=verbosity, dev_var=dev_var)
            if train_acc > 0.99:
                break
            if verbosity > 0:
...
def epoch_fit(model, data, optimizer=None, quan_paras=None, verbosity=0, dev_var=0):
