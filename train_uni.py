...
def UTrain(epochs, header, dev_var=0.0, verbose=False):
    best_acc = 0.0
    for i in range(epochs):
        model.train()
        running_loss = 0.
...
if __name__ == "__main__":
    ...
    UTrain(args.train_epoch, header, dev_var=args.train_var, verbose=args.verbose)
    ...
    model.clear_mask()
    model.to_first_only()
    performance = UEachEval(args.dev_var)
