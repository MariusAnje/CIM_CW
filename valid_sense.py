...
if __name__ == "__main__":
    ...
    parser.add_argument('--slice', action='store',type=int, default=1,
            help='activation slice')
    parser.add_argument('--total_slices', action='store',type=int, default=9,
            help='total nubmer of activation slice')
    ...
    step = int(np.ceil(FEATURESIZE / args.total_slices))
    start_id = (args.slice - 1) * step
    end_id = min(args.slice * step, FEATURESIZE)
    for sl_id in range(start_id, end_id):
        print(sl_id)
        model.de_select_drop()
        model.drop_feature.mask[sl_id] = 0
        crr_acc = CEval()
