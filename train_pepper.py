...
def PTrain(epochs, header, dev_var=0.0, rate=0.0, verbose=False):
    best_acc = 0.0
    for i in range(epochs):
        model.train()
        running_loss = 0.
        ...
        for images, labels in trainloader:
            ...
            optimizer.step()
            running_loss += loss.item()
        test_acc = PEachEval(dev_var, rate)
        # test_acc = CEval()
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), f"tmp_best_{header}.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rate', action='store', type=float, default=0.03,
            help='pepper rate')
    ...
    PTrain(args.train_epoch, header, dev_var=args.train_var, rate=args.rate, verbose=args.verbose)
    ...
    model.clear_mask()
    model.to_first_only()
    performance = PEachEval(args.dev_var, args.rate)
