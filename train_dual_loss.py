...
from utils import TPMTrain, DMTrain, TCEval, TMEachEval, PGD_Eval, CEval, MEachEval, UpdateBN
from utils import copy_model, LossCrossAct
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', action='store', default="MLP4", choices=["MLP3", "MLP3_2", "MLP4", "LeNet", "CIFAR", "Res18", "TIN", "QLeNet", "QCIFAR", "QRes18", "QDENSE", "QTIN", "QVGG", "Adv", "QVGGIN", "QResIN"],
            help='model to use')
    parser.add_argument('--alpha', action='store', type=float, default=1e-6,
            help='weight used in saliency - substract')
    parser.add_argument('--gamma', action='store', type=float, default=1e6,
            help='weight used in saliency - substract')
    ...
    criteriaF = LossCrossAct(alpha=args.alpha, gamma=args.gamma)
    model_group = model, criteriaF, optimizer, scheduler, device, trainloader, testloader
    
    kwargs = {"N":8, "m":1}
    DMTrain(model_group, args.train_epoch, header, args.noise_type, args.train_var, args.rate_max, args.rate_zero, args.write_var, verbose=args.verbose, **kwargs)
