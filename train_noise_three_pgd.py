...
from utils import TPMTrain, MTrain, TCEval, TMEachEval, PGD_Eval, CEval, MEachEval, MEval, MEachEval
...
if __name__ == "__main__":
    ...
    parser.add_argument('--attack_function', action='store', default="act",
            help='learning rate for attack')
    ...
    logger = get_logger(f"logs/PGD/train_noise_three_pgd_log_{args.model}_{args.attack_dist}_{args.rate_max}_{time.time()}")
    logger.info(f"{args}")
    header = time.time()
    header_timer = header
    parent_path = "./"
    ...
    kwargs = {"N":8, "m":1}
    t_model_group = t_model, criteriaF, t_optimizer, w_optimizer, t_scheduler, device, trainloader, memory_testloader
    TPMTrain(t_model_group, args.warm_epoch, args.train_epoch, header, 
             args.noise_type, 0.0, args.train_var, args.rate_max, args.rate_zero, args.write_var, 
             args.train_attack_runs, args.attack_dist, logger=logger, verbose=args.verbose, **kwargs)
    ...
    this_accuracy, this_max, this_l2 = PGD_Eval(model_group, args.attack_runs, args.attack_dist, args.attack_function, use_tqdm = args.use_tqdm)
    logger.info(f"WC acc: {this_accuracy:.4f}")
    exit()
